...
import { useRef, useEffect, useCallback, type CSSProperties, useState } from 'react'
import type {
  Scene,
  OrthographicCamera,
  PlaneGeometry,
...
export default function FloatingLines({
  ...
  lineCount: lineCountProp = [6],
  lineDistance: lineDistanceProp = [5],
  ...
  const getLineCount = useCallback(
    (waveType: 'top' | 'middle' | 'bottom') => {
      if (typeof lineCountProp === 'number') return lineCountProp
      const index = enabledWaves.indexOf(waveType)
      return index >= 0 ? (lineCountProp[index] ?? 6) : 0
    },
    [lineCountProp, enabledWaves]
  )

  const getLineDistance = useCallback(
    (waveType: 'top' | 'middle' | 'bottom') => {
      if (typeof lineDistanceProp === 'number') return lineDistanceProp
      const index = enabledWaves.indexOf(waveType)
      return index >= 0 ? (lineDistanceProp[index] ?? 0.1) : 0.1
    },
    [lineDistanceProp, enabledWaves]
  )
  ...
  useEffect(() => {
    ...
    import('three').then(
      ({
        ...
        let camera: InstanceType<typeof OrthographicCamera>
        ...
        const topLineDistanceVal =
          (enabledWaves.includes('top') ? getLineDistance('top') : 0.1) * 0.01
        const middleLineDistanceVal =
          (enabledWaves.includes('middle') ? getLineDistance('middle') : 0.1) * 0.01
        const bottomLineDistanceVal =
          (enabledWaves.includes('bottom') ? getLineDistance('bottom') : 0.1) * 0.01
        ...
        // Store cleanup so we can call it when the outer effect unmounts
        ;(container as HTMLDivElement & { _fl_cleanup?: () => void })._fl_cleanup = cleanup
      }
    )
    ...
    return () => {
      ...
      if (el) {
        const cleanup = (el as HTMLDivElement & { _fl_cleanup?: () => void })._fl_cleanup
        if (cleanup) cleanup()
      }
    }
  }, [
